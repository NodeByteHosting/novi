...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  try {
    ...
    // Log to database
    await db.logModeration('purge', interaction.guild!.id, interaction.user.id, interaction.guildId!, `Purged ${deleted} messages${targetUser ? ` from ${targetUser.tag}` : ''} in ${channel.name}`);
    ...
    if (logsChannelId) {
      ...
      if (logsChannel) {
        const logEmbed = new EmbedBuilder()
          ...
          .setDescription(`**${deleted}** message${deleted !== 1 ? 's' : ''} deleted${targetUser ? ` from ${targetUser.tag}` : ''} in ${channel.toString()}`)
          .addFields(
