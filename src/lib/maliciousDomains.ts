/**
 * Malicious domain/link list for Discord bot protection
 * Sourced from community reports and security research
 * Last updated: 2026-02-28
 */

export const MALICIOUS_DOMAINS = [
  '101nitro.com',
  '1nitro.club',
  'anonfiles.com',
  'anonymfiles.com',
  'app-nitro.com',
  'appnitro-discord.com',
  'asstralissteam.org.ru',
  'astolfo.live',
  'bit.do',
  'bit.ly',
  'bitly.su',
  'bonus.vin',
  'boost-discord.com',
  'boost-nitro.shop',
  'boostnitro.com',
  'boostnltro.com',
  'boosts-nitro.com',
  'catsnthing.com',
  'catsnthings.fun',
  'clck.ru',
  'crypto-changes.com',
  'cswave.live',
  'curiouscat.club',
  'cutt.ly',
  'dicord.gift',
  'dicsord-get.xyz',
  'dicsord-gift.com',
  'dicsord-gifte.ru',
  'dicsord-give.com',
  'dicsord-nitro.com',
  'dicsord-ticket.com',
  'dicsord.gq',
  'dicsord.net',
  'dicsord.space',
  'dicsord.store',
  'dicsordgift.com',
  'dicsordnitro.com',
  'diiscord-apps.ru',
  'diiscord-codes.ru',
  'diiscord-niitro.ru',
  'diiscord-nitro.ru',
  'diiscord-nittro.ru',
  'diiscord.ru',
  'dilscord.com',
  'dilscordix.net.ru',
  'disc0rd.ru',
  'disccord-apps.com',
  'disccord-apps.ru',
  'disccord-club.com',
  'discoclapp.pw',
  'discocord.gift',
  'discocrd-gift.com',
  'discocrd-gifts.com',
  'discocrd-nitro.com',
  'discocrd.gift',
  'discocrdapp.com',
  'discod.art',
  'discod.info',
  'discodiapp.fun',
  'discodnitro.info',
  'discond.gift',
  'discorb-nitro.ru.com',
  'discorb.co',
  'discorcd-app.com',
  'discorcd-apps.com',
  'discorcd-gift.com',
  'discorcl-app.com',
  'discorcl-boost.ru',
  'discorcl-gift.org.ru',
  'discorcl.click',
  'discorcl.clickn1tro',
  'discorcl.club',
  'discorcl.link',
  'discorcl.shop',
  'discorclapp.com',
  'discorclgift.com',
  'discorclgifts.store',
  'discord-air.site',
  'discord-airdrop.com',
  'discord-airdrop.fun',
  'discord-alrdrop.com',
  'discord-app.me',
  'discord-app.net',
  'discord-appnitro.com',
  'discord-claim.com',
  'discord-codes.ru.com',
  'discord-cpp.com',
  'discord-get.club',
  'discord-get.shop',
  'discord-get.xyz',
  'discord-gg.ru.com',
  'discord-gift.net.ru',
  'discord-gifte.com',
  'discord-gifts.net.ru',
  'discord-gifts.shop',
  'discord-give.com',
  'discord-give.net',
  'discord-give.org',
  'discord-glft.com',
  'discord-infoapp.xyz',
  'discord-informations.ru',
  'discord-niitro.ru',
  'discord-nitr.ru.com',
  'discord-nitro.click',
  'discord-nitro.link',
  'discord-nitro.net',
  'discord-nitro.pro',
  'discord-nitro.pw',
  'discord-nitro.ru',
  'discord-nitro.ru.com',
  'discord-nitro.su',
  'discord-nitroredeem.xyz',
  'discord-promo.xyz',
  'discord-steam-promo.com',
  'discord-steam.com',
  'discord.giveawey.com',
  'discord.gq',
  'discord.moscow',
  'discord.qg',
  'discorda.xyz',
  'discordapp.click',
  'discordapp.one',
  'discordc.gift',
  'discordcommunlty.com',
  'discordd.gift',
  'discordg.link',
  'discordgapp.com',
  'discordgg.ga',
  'discordgift.app',
  'discordgift.info',
  'discordgift.org',
  'discordgift.ru.com',
  'discordgift.site',
  'discordgifte.com',
  'discordgiftss.com',
  'discordgivenitro.com',
  'discordj.gift',
  'discordn.gift',
  'discordnitro.click',
  'discordnitro.com',
  'discordnitro.fun',
  'discordnitro.ru',
  'discordqr.com',
  'discordrgift.com',
  'discords-gift.com',
  'discords-steam.com',
  'discordsgift.com',
  'discordsteams.com',
  'discorl.com',
  'discorrd.ru',
  'discort-nitro.com',
  'discrod-app.com',
  'discrod-gifte.com',
  'discrod-gifts.club',
  'discrod-gifts.com',
  'discrod.fun',
  'discrodapp.ru',
  'discrodapp.site',
  'discrodnitro.org',
  'discrodsteam.ru',
  'diskord.ru.com',
  'disocrd.gifts',
  'disocrd.ru',
  'disord.gifts',
  'disscord-nitro.com',
  'dizcord.gift',
  'dlcsorcl.com',
  'dlcsord-airdrop.com',
  'dlcsord-gift.ru.com',
  'dliscord-giveaway.ru',
  'dliscord.com',
  'dliscord.gift',
  'dliscord.shop',
  'dliscord.xyz',
  'dliscordl.com',
  'dliscordnltro.com',
  'dliscords.com',
  'dlsccord-app.club',
  'dlsccord-app.com',
  'dlsccord-apps.club',
  'dlsccordapps.club',
  'dlsccordclub.com',
  'dlscocrd.com',
  'dlscocrd.ru',
  'dlscocrdapp.com',
  'dlscorcl.gift',
  'dlscord-app.com',
  'dlscord-app.info',
  'dlscord-app.net',
  'dlscord-app.xyz',
  'dlscord-claim.com',
  'dlscord-get.xyz',
  'dlscord-gifts.com',
  'dlscord-niitro.com',
  'dlscord-niitro.ru',
  'dlscord-nitro.click',
  'dlscord-nitro.info',
  'dlscord-nitro.link',
  'dlscord-nitro.ru',
  'dlscord-nltro.com',
  'dlscord-promo.xyz',
  'dlscord-store.club',
  'dlscord.app',
  'dlscord.co.uk',
  'dlscord.gifs.ru.com',
  'dlscord.gift',
  'dlscord.gifts',
  'dlscord.help',
  'dlscord.info',
  'dlscord.ink',
  'dlscord.net',
  'dlscord.org',
  'dlscord.pro',
  'dlscord.ru',
  'dlscord.ru.com',
  'dlscordapp.info',
  'dlscordapps.com',
  'dlscordniltro.com',
  'dlscordnitro.com',
  'dlscordnitro.ru',
  'dlscordsteam.com',
  'dlsocord.com',
  'dlsocrd.com',
  'dlsxorcl.gift',
  'dropmefiles.com',
  'ds-nitro.com',
  'dsicord.gift',
  'ence-official.xyz',
  'ezence.org',
  'fnaticprize.site',
  'fnatik.live',
  'fortnite-newswapper.fun',
  'free-nitroi.ru',
  'freegiftcards.co',
  'gave-nitro.com',
  'get-nitro.net',
  'gg.gg',
  'gift-discord.shop',
  'gifts-discord.xyz',
  'giiveaway-discord.com',
  'giveawaynitro.com',
  'glets-nitro.com',
  'glfte.com',
  'goo.su',
  'grabify.link',
  'hallowen-nitro.com',
  'joinmy.site',
  'lanskoycase.live',
  'leancoding.co',
  'nitro-app-store.com',
  'nitro-discord.org',
  'nitro-discord.site',
  'nitro-discord.store',
  'nitro-discordapp.com',
  'nitro-drop.com',
  'nitrodiscord.org',
  'nitrodsgiveways.com',
  'nitrowithsteam.com',
  'phenix-coders.us',
  'ramlucky.xyz',
  'rust-way.com',
  'simplewallet.cc',
  'sswapper.com',
  'staemcommunity.ru',
  'staemcomunity.ru',
  'steam-discord.fun',
  'steam-discords.com',
  'steam-nitro.online',
  'steam-nitros.ru',
  'steam-nltro.com',
  'steam-nltro.ru',
  'steam-poweredtrades.xyz',
  'steamcconuunity.co',
  'steamcomminnty.com',
  'steamcomminutiu.ru',
  'steamcomminuty.click',
  'steamcomminuty.com',
  'steamcomminytiy.ru',
  'steamcommmunlity.com',
  'steamcommnuntiy.com',
  'steamcommrnunity.com',
  'steamcommunitlu.com',
  'steamcommunity-nitro.ru',
  'steamcommunity.link',
  'steamcommunityu.com',
  'steamcommunityx.com',
  'steamcommunlty.pro',
  'steamcommunlty.ru',
  'steamcommuntry.ru',
  'steamcommunytiu.com',
  'steamcomnmuituy.com',
  'steamcomrninuty.link',
  'steamcomuniity.ru.com',
  'steamcomunity-nitro-free.ru',
  'steamcummonity.com',
  'steamdiscord.com',
  'steamdiscrod.ru',
  'steamdlscord.com',
  'steamdlscords.com',
  'steamnitro.com',
  'steamnitros.ru',
  'steamnltros.com',
  'steamnltros.ru',
  'steams-discord.ru',
  'steancommuniit.ru',
  'steancommunity.link',
  'steancomunnity.ru',
  'stearmcommunitty.ru',
  'stearmcommunity.ru',
  'stearmcormnnuity.ru',
  'stearmcornmunity.ru',
  'stearncommunytiy.ru',
  'stearncomrunity.ru',
  'stearnconrmunity.ru',
  'stearncormmunity.com',
  'stearncormnnuity.ru',
  'stearncormunsity.com',
  'stearncornmnuity.ru',
  'stearncornmumity.ru',
  'stemcornmunity.com',
  'stermccommunitty.ru',
  'stiemcommunitty.ru',
  'stopify.co',
  'store-steampowereb.com',
  'streamcomunnitty.ru',
  'streancommunuty.ru',
  'strearmcommunity.ru',
  'strearmcomunity.ru',
  'surl.li',
  'tournament-predator.xyz',
  'ttr-discord.com',
  'twitch-facepanch.com',
  'twitch-starter.com',
  'twitch.rust-ltd.com',
  'ufile.io',
  'xpro.gift',
  // Fake MrBeast crypto-casino giveaway scams (rotating domains, same template)
  'zzgamb.cc',
  'meakux.com',
  'mervex.com',
  'rosswin.com',
];
